...
const Banner: React.FC<BannerProps> = () => {
    ...
    return (
        <section className='bg-neutral-900 h-[790px]'>
            <div className='container mx-auto h-full'>
                <div className='flex items-center h-full relative -space-x-48 lg:-space-x-24'>
                    <div className='text-white flex-1 z-10 pl-6 lg:pl-0'>
                        <h1 className='text-white mb-8' data-aos='fade-down' data-aos-delay='500'>
                            {titlePart1}<br/>
                            <span className='text-green-400'>{titlePart2}</span>
                        </h1>
                        <p className='max-w-[415px] text-body-md lg:text-body-lg mb-8'
                        data-aos='fade-down' data-aos-delay='600'
                        >{subtitle}</p>
                        <button 
                        className='btn btn-sm lg:btn-lg btn-sec'
                        data-aos='fade-down' data-aos-delay='700'
                        >{textBtn}</button>
                    </div>
                    {/* Image */}
                    <div 
                    className='bg-blue-300 w-full h-full  bg-cover bg-right lg:bg-center bg-no-repeat flex-1'
                    data-aos='fade-right' data-aos-delay='900'
                    >
                        {/* Usa la variable 'img' aquí según sea necesario */}
                    </div>
                </div>
            </div>
        </section>
